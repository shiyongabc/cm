...
RUN CM_VER=5.10.0 && MYSQL_VER=5.7.21 && apk --update add --no-cache wget curl && mkdir -p /rpm && cd /rpm \
 ...
 && tar zxf mysql-connector-java-${MYSQL_VER}.tar.gz && mv -f mysql-connector-java-5.7.21/mysql-connector-java-${MYSQL_VER}-bin.jar ./mysql-connector-java.jar &&rm -rf mysql-connector-java-${MYSQL_VER} mysql-connector-java-${MYSQL_VER}.tar.gz \
